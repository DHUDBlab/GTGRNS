...
parser.add_argument('--net', type=str, default='Non-Specific', help='network type')
...
def train_val_test_set(label_file,Gene_file,TF_file,train_set_file,val_set_file,test_set_file,density,p_val=args.p_val):
    ...
    # 一个TF可能调控多个Target，i/j分别对应TF和Target基因
    ...
    for k in pos_dict.keys():
        ''' 对入度小于等于1的target基因（基因对）的划分，采用随机划分的方法 '''
        if len(pos_dict[k]) <= 1:
            ...
            ''' 对出度为2的TF基因，对应的基因对均等的划分到训练集和测试集当中 '''
            train_pos[k] = [pos_dict[k][0]]
            test_pos[k] = [pos_dict[k][1]]

        else:
            ''' 剩余的基因对按照2：1的比例进行训练集和测试集的划分; 其中训练集按1：4划分成验证集和训练集 '''
            np.random.shuffle(pos_dict[k])
            train_pos[k] = pos_dict[k][:len(pos_dict[k]) * 2 // 3]      #  ’//‘ 表示先做除法然后向下取整
    ...
    for k in train_pos.keys():
        ...
        for i in range(len(train_pos[k])):
            ...
            # 用作生成负样本的Target一定要满足以下条件（Target不能与tf相同，不能出现在Label中，不能出现重复）否则重新筛选直至满足条件为止
...
if __name__ == '__main__':
    ...
    TF2file = '../Dataset' + '/' + 'Benchmark Dataset/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/TF.csv'
    Gene2file = '../Dataset' + '/' + 'Benchmark Dataset/' + '/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/Target.csv'
    label_file = '../Dataset' + '/' + 'Benchmark Dataset/' + '/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/Label.csv'

    train_set_file = '../Dataset' + '/' + 'Benchmark Dataset/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/Train_set.csv'
    test_set_file = '../Dataset' + '/' + 'Benchmark Dataset/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num)  + '/Test_set.csv'
    val_set_file = '../Dataset' + '/' + 'Benchmark Dataset/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) +  '/Validation_set.csv'
